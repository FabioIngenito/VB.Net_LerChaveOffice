...
Friend Class frmLerChaveOffice
    ...
    '
    'Para verificar a chave do caminho que está armazenada para o servidor, inicie
    ' o Editor do Registro do Windows, digitando regedit no menu Iniciar e depois na
    ' caixa de diálogo Executar. Navegue até a chave HKEY_CLASSES_ROOT\Clsid.
    'Nessa chave você encontrará os CLSIDs para os servidores de automação
    ' registrados no sistema. Depois, usando os valores, encontre a chave que
    ' representa o aplicativo do Office que você quiser automatizar e verifique o
    ' caminho da chave LocalServer32 do mesmo.

    'To verify the path key that Is stored for the server, start 
    ' the Windows Registry Editor by typing regedit On the Start menu And Then In 
    ' the Run dialog box. Navigate To the HKEY_CLASSES_ROOT \ Clsid key.
    'In this key you will find the CLSIDs for the automation servers 
    ' registered in the system. Then, using the values, find the key that 
    ' represents the Office application that you want To automate, And check the 
    ' path Of the LocalServer32 key Of the Office application.
    '
    '   +========================+=========================================+
